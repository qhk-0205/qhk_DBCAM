...
class Config:
    ...
    modelname = "Ours"   # 目前改进的模型用Ours调用，采取的是DenseNet169+SE，移除了ECA模块
    num_classes = 12  
    preload_images = True  
    ...
    result_save_dir = f"1_22_DenseNet169+SE_Final_results/{modelname}-{n_folds}fold-{minority_aug_times}-{lr_scheduler_monitor}_{batch_size}-{epochs}-{lr}-1"
...
def get_model(model_name, num_classes):
    model_dict = {
        ...
        "Ours": lambda: densenet169NoECA(num_classes=12)
        }
