...
class Config:
    ...
    batch_size = 32  # 批次大小
    epochs = 25  # 训练轮数
    label_root_dir = r"E:\ResNet\ResNet\Sheep_Coccidia_Final_Data_Old2_1_10\Data"  #图像总目录，12个类别文件夹的父目录
    
    progress_interval = 5  #每5个批次打印进度
    img_formats = ['.jpg', '.jpeg', '.png', '.bmp']
    modelname = "Ours"   # 目前改进的模型用Ours调用，采取的是DenseNet169+ECA,移除了双通道注意力中的SE模块
    num_classes = 12  
    preload_images = True  
    ...
    result_save_dir = f"1_22_DenseNet169+ECA_Final_results/{modelname}-{n_folds}fold-{minority_aug_times}-{lr_scheduler_monitor}_{batch_size}-{epochs}-{lr}-1"
...
def densenet169NoSE(num_classes):
...
def get_model(model_name, num_classes):
    model_dict = {
        ...
        "Ours": lambda: densenet169NoSE(num_classes=12)
        }
